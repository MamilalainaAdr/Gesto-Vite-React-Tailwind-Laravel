...
export default function ModifCommande ({cancel, confirm, isOpen, update, commande}){
    ...
    useEffect(()=>{
        setSelectedFournisseur(commande?.ID_Fournisseur)
        setSelectedProduit(commande?.ID_Produit)
        setQuantite(commande?.Qte_Commande)
        setDate(commande?.Date_Commande)
    }, [commande])
        ...
        if (isInvalid) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    messageAPI.success('Commande modifier avec succès')
    ...
    return(<>
        {contextHolder}
        <Modal title="Modifier Commande" okText='Ajouter' onCancel={cancel} onOk={handleSubmit} open={isOpen}>
